...
- 默认目录分为四个“通用整理方案”和三个“官方规范参考包”。每项都携带版本、维护方、复核日期、
  来源、能力覆盖和明确限制；参考包还展示逐条覆盖矩阵与自动验收证据，且不会被系统自动推荐。
...
- **我的规则包**：把当前规则命名保存到本地独立规则库，保留不可变修订、适用范围、变更说明、
  本地核对记录和 SHA-256；可载入旧版、导出版本化 JSON、先检查再跨设备导入，或把旧版恢复为
  新的待确认草稿。导入不会覆盖同名规则，完全相同的工件也不会重复创建。
- 用户规则始终优先于预设规则；高级用户也可以直接编辑 JSON/YAML 规则。

规则包保存、创建新修订、恢复和跨设备导入都带幂等请求标识。若响应返回前连接中断，使用同一内容
重试会返回原结果，不会产生重复修订。跨机文件会先校验严格结构与规则摘要；当前工件没有数字签名，
来源电脑的“本地已确认”只保留为追溯信息，导入后固定重置为草稿，不代表机构认证或法规合规。
...
- 结构分析后同时识别字段、公式、图片、书签、内容控件、合并/嵌套表格、文本框、脚注尾注、批注、
...
- 轮询请求带超时、取消和指数退避，终态后立即停止，不创建重复连接；单文档和批次均提供立即重试。
- 浏览器恢复记录与当前数据目录不一致、或对应文档/批次/任务已被删除时，会按 HTTP 404 自动清除
  失效引用并停止轮询，不再把“记录不存在”误报成网络重连。
...
- “设置”页面按源 DOCX、分析、任务审计、输出、批次包、待完成清理和数据库分类显示本机占用，并提示剩余磁盘空间。
- 页面分别列出已结束批次与未归入批次的文档，先展示可回收空间，再由用户逐项确认删除；默认不会
  自动过期或清理资料。
- 处理中的文档不能删除，避免任务运行时丢失源文件；终态批次删除仍保留跨文档复用的规则包。
- 删除先把关联文件原子移动到同卷隔离区，再提交数据库；中断后启动时按数据库事实恢复或继续清理。
  无法核对的清单保持原状，设置页提供显式“重试安全清理”，待完成期间拒绝生成不一致的完整备份。
- 存储统计不读取文档正文、不把完整本地路径返回浏览器，也不会跟随数据目录中的符号链接。

### 可审阅的本机诊断

- “设置”页面可主动运行数据库连接、迁移版本、数据目录权限、磁盘空间、本地产物引用和删除事务恢复检查，并给出
  普通用户可执行的处理建议。
...
- 完成页把“结构段落待确认”和“交付前规则核对”分开计数，避免把自动格式验证通过误解为整份文档
  已经完整合规。
- 参考包结果会固化引擎版本、执行规则 SHA-256、目录规则 SHA-256、公开来源、覆盖条款和人工验收
  清单；高级 JSON 被修改后会撤销原确认，并明确标记原参考包覆盖结论已经失效。
- 完成页可打开源文档与输出文档的本机并排预览；两侧默认同步滚动，具体改动可返回结构列表高亮，
  独立任务页刷新恢复后也能继续比较。
...
- 段落、文字 run 和表格位置可点击并返回结构列表高亮；完整变更保留在 JSON，Markdown 审计列出
  前 50 项。
- 任务结果通过版本化 OpenAPI 契约恢复；独立任务页会恢复同一份规则证据和交付清单，工作台可从
  结果摘要直接聚焦结构待确认段落。
- 并排视图是浏览器近似渲染，不是 Word 法律黑线、修订记录或像素差异；分页、替代字体、形状和
  文本框仍应在 Word/WPS 中最终复核，机器结论以 OOXML 重开验证和审计为准。

## 内置整理方案与规范参考包

通用整理方案用于快速统一观感，不代表机构合规：
...
通用方案启用内容保护、黑色文字、突出与背景清理，以及保存后重开验证。

官方规范参考包只自动执行覆盖矩阵中的安全子集：

| 参考包 | 当前自动覆盖 | 必须人工处理或复核 |
| --- | --- | --- |
| GB/T 9704—2012 主体参考 | A4 版心、标题与正文的字体字号和段落基线 | 版头、版记、印章、奇偶页码及完整发文要素 |
| 南开大学论文 2026 参考 | 页面边距、四级标题、正文、图表题、参考文献与附录文字 | 封面、目录、分节页码、动态篇眉及中英文摘要标题差异 |
| 北京印刷学院硕士论文 2025 参考 | 页面边距、三级标题、正文、图表题、参考文献与附录文字 | 官方模板、10 mm 装订线、分节页码、动态页眉及三线表细节 |

选择参考包后，应先展开来源、逐条覆盖矩阵和验收证据；只有勾选“我已查看自动、人工和暂不支持
条款”后才能执行格式化。“格式验证通过”仅表示已声明的自动条款和内容保护通过，不表示发布机构
审核、授权或完整合规。
...
3. 检查并修正段落角色。
4. 查看“文档处理范围预检”；存在复杂对象时确认其处理方式和 Word/WPS 人工核对责任。
...
| `POST /api/v1/rule-packs/imports/preview` | 只读检查跨设备规则包、完整性、冲突和重复项 |
...
| `POST /api/v1/workspace/cleanup/retry` | 按数据库状态恢复或继续清理中断的删除事务 |
...
书签、公式、合并及嵌套表格、多节、横竖版、页眉页脚、视觉污染和未知顶层 OOXML。端到端测试使用
临时数据目录、独立端口和生产静态网站，不读取或写入日常工作区数据库。
...
- 用户确认删除后，文件与数据库通过同卷暂存事务保持一致；异常中断会在启动或设置页重试时恢复，
  损坏清单不会被自动猜测处理。普通文件删除不等于敏感介质的合规安全擦除。
- 上传、排版、批次/交付打包和完整备份会先检查目标卷容量，默认保留 1 GiB 安全余量；空间不足统一
  返回可解释的容量错误，不进入网络重连。该余量可通过 `DOCALIGN_MIN_FREE_MB` 调整，不会自动删除数据。
- 设置页可导出完整工作区备份：使用 SQLite 在线快照，收录全部托管文档与产物、版本化清单及
  SHA-256；严格校验后只能恢复到新目录，并自动迁移数据库路径。`.env`、密钥、运行锁、WAL/SHM
  和非托管文件明确排除。备份含敏感原文且未加密、未签名，不能作为发布者身份证明。
